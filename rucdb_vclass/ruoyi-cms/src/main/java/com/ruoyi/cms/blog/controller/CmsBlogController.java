...
public class CmsBlogController extends BaseController {
     ...
     * 首页按分类查询文章列表
     */
    @GetMapping("/cms/cmsListByMenu/{id}")
    public TableDataInfo cmsListByMenuId(@PathVariable(value = "id", required = true) Long id) {
        startPage();
        List<CmsBlog> list = cmsBlogService.selectCmsBlogListByMenuId(id);
        return getDataTable(list);
    }
     ...
     * 修改文章管理
     */
    @PreAuthorize("@ss.hasPermi('cms:blog:approve')")
    @Log(title = "文章管理", businessType = BusinessType.UPDATE)
    @PutMapping("/approve")
    public AjaxResult approve(@RequestBody CmsBlog cmsBlog) {
        CmsBlog oldBlog = cmsBlogService.selectCmsBlogById(cmsBlog.getId());
        if (oldBlog == null) {
            return AjaxResult.error();
        } else if (oldBlog.getStatus().equals("1")) {
            return AjaxResult.error("已发布文章无需审核");
        }
        oldBlog.setStatus("1");

        return toAjax(cmsBlogService.approveBlog(cmsBlog));
    }
