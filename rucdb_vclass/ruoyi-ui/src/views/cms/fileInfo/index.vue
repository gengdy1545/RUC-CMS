<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="fileInfoList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="可下载用户" align="center" prop="onlyLoginCanDownload" width="100">
        <template slot-scope="scope">
          <span v-if="scope.row.onlyLoginCanDownload === 0">所有用户</span>
          <span v-else>登录用户</span>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleUpdate(only) {
      if (!this.ids.length) {
        this.$modal.msgWarning("请选择需要修改的文件！");
        return;
      }

      this.$modal
        .confirm(`是否确认将选中的${this.ids.length}个文件设为仅登录用户可下载？`)
        .then(() => {
          // 使用Promise.all批量更新文件信息
          const updateRequests = this.ids.map(id => {
            return updateFileInfo({ fileId: id, onlyLoginCanDownload: only });
          });

          return Promise.all(updateRequests); // 等待所有更新请求完成
        })
        .then(() => {
          this.getList(); // 重新刷新列表
