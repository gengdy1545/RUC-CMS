    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
                   v-hasPermi="['cms:blog:add']">新增
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" plain icon="el-icon-edit" size="mini" :disabled="single" @click="handleUpdate"
                   v-hasPermi="['cms:blog:edit']">修改
        </el-button>
      </el-col>
      <el-col :span="1.5">
    ...
    <el-table v-loading="loading" :data="blogList" @selection-change="handleSelectionChange"
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          ...
          <v-if v-show="scope.row.status == '2'">
            <el-button size="mini" type="text" icon="el-icon-folder-opened" @click="handleApprove(scope.row)"
                       v-hasPermi="['cms:blog:approve']">复核
            </el-button>
          </v-if>
          <el-button size="mini" type="text" icon="el-icon-folder-opened" @click="blogFiles(scope.row)"
                     v-hasPermi="['cms:blog:edit']">附件管理
          </el-button>
    ...
    <el-dialog :title="title" :visible.sync="open" :before-close="cancel" width="1200px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-row>
          <el-col :span="8">
            <el-form-item label="首图">
              ...
              <div v-show="form.blogPicType == '0'" class="tabBlock">
                <el-input v-model="form.blogPicLink" placeholder="请输入图片地址 https://"
          ...
          <el-col :span="16">
            <el-form-item label="简介">
              <el-input type="textarea" v-model="form.blogDesc" :autosize="{ minRows: 7, maxRows: 7}" maxlength="50"
        ...
        <el-form-item label="内容">
          ...
          <el-row>
            <el-col>
              <cmsEditor v-if="form.contentType ==='1'" v-model="form.content" @getFileId="getFileId" type="base64"
                         :min-height="192"/>
              <CherryMarkdown ref="CherryMarkdown" v-if="form.contentType ==='2'" :height='400'
                              v-model='form.contentMarkdown'></CherryMarkdown>
          ...
          <el-row>
            ...
            <el-table :data="form.blogFilesNew" :border="true" style="width: 99.99%;">
              ...
              <el-table-column align="center" min-width="20%" label="操作">
                <template slot-scope="scope">
                  <el-button v-show="scope.row.fileId!==''" size="mini" plain @click="handleDownload(scope.row)">下载
                  </el-button>
...
import {treeselect as menuTreeselect, blogMenuTreeselect} from "@/api/system/menu";
import {
  listBlog,
  getBlog,
  delBlog,
...
export default {
  ...
  methods: {
    ...
    getBlogMenuTreeselect(blogId) {
      return blogMenuTreeselect(blogId).then(response => {
    ...
    handleUpdate(row) {
      ...
      const blogMenu = this.getBlogMenuTreeselect(id);
    ...
    handleApprove(row) {
      const ids = row.id || this.ids;
      let name = row.title || this.names;
      this.$modal.confirm('是否确认通过"' + name + '"？').then(() => {
        return approveBlog({
          id: ids,
          status: 1
        });
